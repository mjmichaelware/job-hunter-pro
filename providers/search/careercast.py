"""CareerCast — keyless career source.\n\nProfessional and specialized job listings"""
import logging
...
class CareerCastProvider(SearchProvider):
    @property
    def metadata(self) -> ProviderMetadata:
        return ProviderMetadata(
            ...
            description="Professional and specialized job listings",
    ...
    def search(self, query: str, location: str = "", limit: int = 50) -> List[SearchResult]:
        ...
        try:
            url = "https://www.careercast.com/jobs/api/search?keyword=%s&location=Salt+Lake+City%%2C+UT" % quote_plus(query.strip())
            ...
            for it in items[:limit]:
                ...
                results.append(SearchResult(
                    ...
                    url=str(it.get("url") or it.get("link") or "" if isinstance(it, dict) else "") or url,
                    snippet=(str(it.get("company","") if isinstance(it, dict) else "")[:300] or None),
                    source_name=it.get("company") or "CareerCast" if isinstance(it, dict) else "CareerCast", published_date=it.get("date") if isinstance(it, dict) else None,
            ...
            log.error("CareerCast search failed: %s", e)
        return results


careercast_provider = CareerCastProvider()

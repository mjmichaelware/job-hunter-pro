...
def canonical_key(job: Dict[str, Any]) -> str:
    return "|".join([
        ...
        clean(job.get("restaurant_name")).lower(),
        clean(job.get("resolved_address")).lower(),
    ])
...
def apply_filters(jobs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for job in jobs:
        ...
        if max_radius is not None:
            try:
                if float(job.get("radius_miles") or 999) > max_radius:
                    continue
            except Exception:
                continue
        if max_transit is not None:
            try:
                if float(job.get("commute_seconds") or 999999) / 60 > max_transit:
                    continue
            except Exception:
                continue

...
echo "=== SHOW ALL LIVE JOB CANDIDATES ==="
echo "Accepted jobs stay verified. Rejected/unresolved candidates become visible instead of hidden."
echo "PWD=$(pwd)"
echo
...
echo "=== 1) Backup current renderer ==="
mkdir -p .repair_backups
TS="$(date +%Y%m%d_%H%M%S)"
cp web/static/js/render_jobs.js ".repair_backups/render_jobs.show_all_candidates.${TS}"

echo
echo "=== 2) Replace jobs renderer with accepted + unresolved candidate rendering ==="
...
echo
echo "=== 3) Post-patch inspection ==="
grep -Rni "All Live Job Candidates\|Unresolved live candidates\|normalizeRejectedPayload\|renderUnresolvedCandidateCard\|provider_breakdown\|Needs resolution" web/static/js/render_jobs.js

echo
echo "=== 4) Compile locally ==="
python3 -m py_compile $(git ls-files '*.py')

echo
echo "=== 5) Local proof without live /api/jobs ==="
python3 - <<'PY'
from app import app

c = app.test_client()

for path in ["/", "/api/health", "/api/jobs?dry_run=1", "/static/js/render_jobs.js"]:
    r = c.get(path)
    print(path, r.status_code, r.content_type)
    assert r.status_code == 200

js = c.get("/static/js/render_jobs.js").get_data(as_text=True)
assert "All Live Job Candidates" in js
assert "Unresolved live candidates" in js
assert "normalizeRejectedPayload" in js
assert "renderUnresolvedCandidateCard" in js
assert "Needs resolution" in js
assert "const url = live ? API_URLS.jobs" in js
assert "?dry_run=1" in js

print("PASS: frontend will show accepted jobs and unresolved rejected live candidates.")
PY

echo
echo "=== 6) Diff proof ==="
git diff -- web/static/js/render_jobs.js
git diff --check

echo
echo "=== 7) Commit and push ==="
git add web/static/js/render_jobs.js
git commit -m "Show all live job candidates"
git push origin main

echo
echo "=== 8) Wait for deploy trigger ==="
sleep 120

echo
echo "=== 9) Verify Cloud Run ==="
...
echo
echo "=== 11) Live JS proof ==="
curl -fsS "$SERVICE_URL/static/js/render_jobs.js?v=$(date +%s)" \
  | grep -E "All Live Job Candidates|Unresolved live candidates|normalizeRejectedPayload|renderUnresolvedCandidateCard|Needs resolution" || true

echo
echo "OPEN:"
echo "$SERVICE_URL/?v=show-all-candidates-$(date +%s)"
echo
echo "Press Run Live Discovery. The UI should now show accepted verified jobs plus unresolved live candidates."

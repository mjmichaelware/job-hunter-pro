"""College Recruiter — keyless career source.\n\nEntry-level and student job listings"""
import logging
...
class CollegeRecruiterProvider(SearchProvider):
    @property
    def metadata(self) -> ProviderMetadata:
        return ProviderMetadata(
            key="college_recruiter",
            label="College Recruiter",
            type=ProviderType.SEARCH,
            description="Entry-level and student job listings",
    ...
    def search(self, query: str, location: str = "", limit: int = 50) -> List[SearchResult]:
        ...
        try:
            url = "https://www.collegerecruiter.com/api/jobs?keywords=%s&location=Salt+Lake+City%%2C+UT&radius=25" % quote_plus(query.strip())
            ...
            for it in items[:limit]:
                ...
                results.append(SearchResult(
                    ...
                    source_name=it.get("company") or "College Recruiter" if isinstance(it, dict) else "College Recruiter", published_date=it.get("posted") if isinstance(it, dict) else None,
            ...
            log.error("College Recruiter search failed: %s", e)
        return results


college_recruiter_provider = CollegeRecruiterProvider()

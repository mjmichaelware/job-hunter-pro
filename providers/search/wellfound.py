"""Wellfound — keyless career source.\n\nStartup and tech company job listings"""
import logging
...
class WellfoundProvider(SearchProvider):
    @property
    ...
    def search(self, query: str, location: str = "", limit: int = 50) -> List[SearchResult]:
        ...
        try:
            url = "https://api.wellfound.com/jobs?location=salt-lake-city&keywords=%s&per_page=%d" % (quote_plus(query.strip()), limit)
            ...
            for it in items[:limit]:
                ...
                if not title:
                    continue
                hay = " ".join(str(x) for x in [it.get("title",""), it.get("startup_name",""), it.get("location","")]).lower()
                ...
                results.append(SearchResult(
                    ...
                    snippet=(str(it.get("startup_name","") if isinstance(it, dict) else "")[:300] or None),
                    source_name=it.get("startup_name") or "Wellfound" if isinstance(it, dict) else "Wellfound", published_date=it.get("created_at") if isinstance(it, dict) else None,
            ...
            log.error("Wellfound search failed: %s", e)
        return results

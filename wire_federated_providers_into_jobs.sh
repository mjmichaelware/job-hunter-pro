...
curl -fsS "$SERVICE_URL/api/providers" -o live_providers_after_fanout.json
python3 - <<'PY'
import json
data = json.load(open("live_providers_after_fanout.json"))
...
OUT="./live_jobs_federated_now.json"
rm -f "$OUT"
...
data = json.loads(Path("live_jobs_federated_now.json").read_text())
